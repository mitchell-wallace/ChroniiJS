...
export function registerIpcHandlers(): void {
	...
	ipcMain.handle(
		'timer:start',
		async (_, taskName: string): Promise<TimeEntry> => {
			const db = await getDatabase();
	...
	ipcMain.handle(
		'entries:get-all',
		async (_, limit?: number, offset?: number): Promise<TimeEntry[]> => {
			const db = await getDatabase();
	...
	ipcMain.handle(
		'entries:get-range',
		async (_, startDate: number, endDate: number): Promise<TimeEntry[]> => {
			const db = await getDatabase();
	...
	ipcMain.handle(
		'db:info',
		async (): Promise<{ path: string; isOpen: boolean }> => {
			const db = await getDatabase();
			return db.getInfo();
		},
	);
	...
	ipcMain.handle('config:set', async (_, config: ChroniiConfig) => {
		return setConfig(config);
	});

	ipcMain.handle('config:update', async (_, updates: ChroniiConfigUpdate) => {
	...
	ipcMain.handle(
		...
		async (_, backupPath: string, mode: RestoreMode) => {
	...
	ipcMain.handle('db:select-export-path', async (_, suggestedName?: string) => {
		...
		const result = await dialog.showSaveDialog({
			defaultPath: dbDir
				? path.join(dbDir, suggestedName ?? 'chronii-database.db')
				: (suggestedName ?? 'chronii-database.db'),
	...
	ipcMain.handle(
		'db:import-csv',
		async (_, sourcePath: string, options?: { dedupe?: boolean }) => {
	...
	ipcMain.handle(
		'db:apply-changes',
		async (_, changes: ApplyChangesPayload) => {
			const db = await getDatabase();
	...
	ipcMain.handle(
		'db:select-export-csv-path',
		async (_, suggestedName?: string) => {
			const dbDir = await getDatabaseDirectory();
			const result = await dialog.showSaveDialog({
				defaultPath: dbDir
					? path.join(dbDir, suggestedName ?? 'chronii-entries.csv')
					: (suggestedName ?? 'chronii-entries.csv'),

...
describe('IPC Handlers Integration Tests', () => {
	...
	describe('Timer Operations', () => {
		it('should start a timer', async () => {
			...
			const entry = await (ipcMain as any)._invokeHandler(
				'timer:start',
				taskName,
			);
		...
		it('should stop active timer when starting new timer', async () => {
			// Start first timer
			const entry1 = await (ipcMain as any)._invokeHandler(
				'timer:start',
				'Task 1',
			);
			...
			// Start second timer
			const entry2 = await (ipcMain as any)._invokeHandler(
				'timer:start',
				'Task 2',
			);
		...
		it('should stop a timer', async () => {
			...
			const stopped = await (ipcMain as any)._invokeHandler(
				'timer:stop',
				entry.id,
			);
	...
	describe('Entry CRUD Operations', () => {
		...
		it('should get entry by ID', async () => {
			const created = await (ipcMain as any)._invokeHandler(
				'timer:start',
				'Test Task',
			);
		...
		it('should update entry', async () => {
			...
			const updated = await (ipcMain as any)._invokeHandler(
				'entries:update',
				entry.id,
				{
	...
	describe('Range Query Operations', () => {
		it('should get entries in range', async () => {
			...
			expect(entries).toHaveLength(2);
			expect(entries.some((e: TimeEntry) => e.taskName === 'Task 2')).toBe(
				true,
			);
			expect(entries.some((e: TimeEntry) => e.taskName === 'Task 3')).toBe(
				true,
			);
		});

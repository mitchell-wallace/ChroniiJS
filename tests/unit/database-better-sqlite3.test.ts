...
import { BetterSQLiteDatabaseService } from '../../electron/database-better-sqlite3';
...
vi.mock('electron', () => ({
	app: {
		getPath: (name: string) => {
			if (name === 'userData') {
				return path.join(os.tmpdir(), `chronii-test-${Date.now()}`);
			}
			return os.tmpdir();
		},
	},
...
describe('BetterSQLiteDatabaseService', () => {
	let db: BetterSQLiteDatabaseService;
	let testDbPath: string;

	beforeEach(() => {
		// Create a fresh database instance for each test
		db = new BetterSQLiteDatabaseService();
	...
	describe('getActiveTimeEntry', () => {
		...
		it('should return most recent active entry when multiple exist', () => {
			const _entry1 = db.createTimeEntry('Task 1', Date.now() - 1000);
			const entry2 = db.createTimeEntry('Task 2', Date.now());
	...
	describe('getAllTimeEntries', () => {
		...
		it('should respect offset parameter', () => {
			const entry1 = db.createTimeEntry('Task 1', Date.now() - 2000);
			const entry2 = db.createTimeEntry('Task 2', Date.now() - 1000);
			const _entry3 = db.createTimeEntry('Task 3', Date.now());

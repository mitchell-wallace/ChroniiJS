...
const seedEntries = async (page: any, count: number) => {
	await page.evaluate(async (entryCount) => {
		...
		for (let i = 0; i < entryCount; i += 1) {
			...
			await w.entriesAPI.updateEntry(entry.id, {
				startTime,
				endTime,
				logged: false,
			});
...
test('marking logged keeps scroll position in the time list', async ({
	page,
}) => {
	...
	await page.waitForFunction(
		() => (window as any).entriesAPI && (window as any).timerAPI,
	);

...
const WeeklySummary: Component<WeeklySummaryProps> = (props) => {
	...
	return (
		<div
			class="bg-base-300/50"
			data-testid={`weekly-summary-${props.week.weekLabel.toLowerCase().replace(/\s+/g, '-')}`}
		>
			...
			>
				<span
					data-testid={`weekly-label-${props.week.weekLabel.toLowerCase().replace(/\s+/g, '-')}`}
				>
			...
			<div
				data-testid={`weekly-days-${props.week.weekLabel.toLowerCase().replace(/\s+/g, '-')}`}
			>

...
const WebBackupReminderBanner: Component = () => {
  const [reminder, setReminder] = createSignal<ReminderConfig>(DEFAULT_REMINDER);
  ...
  onMount(() => {
    ...
    onCleanup(() => window.removeEventListener('chronii:config-updated', handleConfigUpdate));
  });
    ...
    <Show when={isVisible()}>
      <div class="fixed top-14 left-1/2 -translate-x-1/2 z-40">
        <div class="bg-base-100 border border-base-300 shadow-lg rounded-full px-4 py-2 flex items-center gap-3">
          <div class="text-sm">
            Backup reminder: download your {reminder().format.toUpperCase()} backup today.
          </div>
          <button class="btn btn-xs btn-primary" onClick={handleDownload} title="Download a backup">
            Download backup
          </button>
          <button class="btn btn-xs btn-ghost" onClick={dismissReminder} title="Dismiss reminder">
            Dismiss
          </button>
        </div>
      </div>

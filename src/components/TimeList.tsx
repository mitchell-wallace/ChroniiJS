...
const TimeList: Component<TimeListProps> = (props) => {
	...
	const handleToggleLogged = async (entryId: string) => {
		try {
			...
			const success = await window.entriesAPI.updateEntry(entryId, {
				logged: !entry.logged,
			});
			if (success) {
				await loadEntries();
	...
	const groupedByWeeks = (): WeeklyGroup[] => {
		...
		entriesList.forEach((entry) => {
			...
			let weekGroup = weekGroups.find(
				(w) => w.weekStart.toDateString() === weekKey,
			);
		...
		weekGroups.forEach((weekGroup) => {
			weekGroup.days.sort((a, b) => {
				...
				return (
					new Date(b.entries[0]?.startTime || 0).getTime() -
					new Date(a.entries[0]?.startTime || 0).getTime()
				);
			});
		});
	...
	return (
		<div class="h-full flex flex-col bg-base-100" data-testid="time-list">
				...
				<div class="flex items-center justify-between">
					...
					{entries().length > 0 && (
						<div
							...
							data-testid="time-list-entry-count"
						>
			...
			<ConfirmDialog
				...
				title={
					deleteConfirmData().count === 1
						? `Delete "${deleteConfirmData().items[0]}"?`
						: `Delete ${deleteConfirmData().count} tasks?`
				}
				message="This action cannot be undone."
				items={
					deleteConfirmData().count > 1 ? deleteConfirmData().items : undefined
				}
...
import type { TimeEntryUpdate } from '../shared/api-types';

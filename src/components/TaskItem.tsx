...
import { formatDuration, formatTime } from '../utils/timeFormatting';
		...
		<>
			...
			>
				<Show
					...
					fallback={
						<div
							class="flex items-center gap-1 min-h-[2rem]"
							data-testid={`task-item-${props.entry.id}-display`}
						>
							<div class="flex-1 min-w-0">
								<div
									class={`font-medium text-sm truncate ${isUntitled() ? 'opacity-60 italic' : ''}`}
									data-testid={`task-item-${props.entry.id}-name`}
								>
									{props.entry.taskName}
								</div>
								<div
									class="text-xs text-base-content/60"
									data-testid={`task-item-${props.entry.id}-time-range`}
								>
									{formatTime(props.entry.startTime)} -{' '}
									{props.entry.endTime
										? formatTime(props.entry.endTime)
										: 'Running'}
								</div>
							</div>
							...
							<div
								class="flex -gap-px flex-shrink-0"
								data-testid={`task-item-${props.entry.id}-actions`}
							>
								<button
									type="button"
								...
								<button
									...
									onClick={() => props.onToggleLogged(props.entry.id)}
									title={
										props.entry.logged ? 'Mark as not logged' : 'Mark as logged'
									}
				...
				>
					<div
						...
						data-testid={`task-item-${props.entry.id}-edit-form`}
					>
						<input
							type="text"
						...
						<div
							...
							data-testid={`task-item-${props.entry.id}-edit-actions`}
						>
							<button
								type="button"
							...
							<div class="flex gap-1">
								...
								<button
									...
									disabled={
										!props.editValues.taskName.trim() ||
										!props.editValues.startTime
									}

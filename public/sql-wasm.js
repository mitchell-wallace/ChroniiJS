	...
	initSqlJsPromise = new Promise((resolveModule, reject) => {
		...
		f.onRuntimeInitialized = () => {
			...
			e.prototype.export = function () {
				Object.values(this.fb).forEach((l) => {
					l.free();
				});
			...
			e.prototype.close = function () {
				null !== this.db &&
					(Object.values(this.fb).forEach((g) => {
						g.free();
					}),
					Object.values(this.Sa).forEach(A),
					(this.Sa = {}),
			...
			e.prototype.create_function = function (g, l) {
				Object.hasOwn(this.Sa, g) && (A(this.Sa[g]), delete this.Sa[g]);
				var n = Aa((r, t, y) => {
					t = b(t, y);
					try {
			...
			e.prototype.create_aggregate = function (g, l) {
				...
				Object.hasOwn(this.Sa, g) && (A(this.Sa[g]), delete this.Sa[g]);
				l = `${g}__finalize`;
				Object.hasOwn(this.Sa, l) && (A(this.Sa[l]), delete this.Sa[l]);
				var L = Aa((I, M, Ra) => {
						var X = ub(I, 1);
						Object.hasOwn(y, X) || (y[X] = n());
						M = b(M, Ra);
						M = [y[X]].concat(M);
						try {
							y[X] = t.apply(null, M);
			...
			O = {
				...
				La: {
					...
					rename(a, b, c) {
						...
						if (d) {
							if (P(a.mode)) for (var _e in d.Na) throw new N(55);
							Cb(d);
						}
		...
		function Wb(a, b) {
			...
			a.Xa = b;
			b.root = a;
			c ? (Eb = a) : d && ((d.ab = b), d.Xa?.Ib.push(b));
		}
		function Xb(a, b, c) {
		...
		(() => {
			U('/dev');
			wb(259, { read: () => 0, write: (_d, _e, _h, k) => k, Va: () => 0 });
			Yb('/dev/null', 259);
			nb(1280, yb);
			nb(1536, zb);
			Yb('/dev/tty', 1280);
		...
		var Xc = {
				a: (a, b, c, d) =>
					Ta(
						`Assertion failed: ${a ? B(x, a) : ''}, at: ${[b ? (b ? B(x, b) : '') : 'unknown filename', c, d ? (d ? B(x, d) : '') : 'unknown function']}`,
					),
				i: (a, b) => {
					try {
